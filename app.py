...
def garantir_cabecalhos():
    ...
    try:
        try: ws_prod = sh.worksheet("Perfis_Producao")
        except: ws_prod = sh.add_worksheet(title="Perfis_Producao", rows=1000, cols=20)
        if not ws_prod.row_values(1):
            ws_prod.append_row(["id", "data_hora", "lote", "reserva", "status_reserva", "cod_sap", "descricao", "qtd", "peso_real", "tamanho_real_mm", "tamanho_corte_mm", "peso_teorico", "sucata"])
        
        try: ws_lotes = sh.worksheet("Perfis_Lotes")
        except: ws_lotes = sh.add_worksheet(title="Perfis_Lotes", rows=1000, cols=5)
...
def salvar_no_banco(dados):
    ...
    linha = [
        ...
        int(dados['Qtd']), float(dados['Peso Balança (kg)']), int(dados['Tamanho Real (mm)']),
...
def limpar_banco_completo():
    sh = conectar_google()
    sh.worksheet("Perfis_Producao").clear()
    sh.worksheet("Perfis_Producao").append_row(["id", "data_hora", "lote", "reserva", "status_reserva", "cod_sap", "descricao", "qtd", "peso_real", "tamanho_real_mm", "tamanho_corte_mm", "peso_teorico", "sucata"])
    sh.worksheet("Perfis_Lotes").clear()
    sh.worksheet("Perfis_Lotes").append_row(["cod_sap", "ultimo_numero"])
...
if modo_acesso == "Operador (Chão de Fábrica)":
    ...
    if df_sap is not None:
        ...
        st.text_input("BIPAR CÓDIGO:", key="input_scanner", on_change=check_scan)
    ...
    if st.sidebar.text_input("Senha", type="password") == "Br@met4l":
        ...
        if not df.empty:
            ...
            with t1:
                ...
                ed = st.data_editor(df, use_container_width=True, key="ed_perfis", column_config={
                ...
                for _, r in df.iterrows():
                    ...
                    if r['sucata'] > 0.001:
                        lst.append({'Lote': 'VIRTUAL', 'Reserva': r['reserva'], 'SAP': r['cod_sap'], 'Descrição': f"SUCATA - {r['descricao']}", 'Status': r['status_reserva'], 'Qtd': 1, 'Peso Lançamento (kg)': formatar_br(r['sucata']), 'Comp. Real': 0, 'Comp. Corte': 0})
                
                df_exp = pd.DataFrame(lst)
                if not df_exp.empty:
                    cols_final = [c for c in ['Lote', 'Reserva', 'SAP', 'Descrição', 'Peso Lançamento (kg)', 'Status', 'Qtd', 'Comp. Real (mm)', 'Comp. Corte (mm)'] if c in df_exp.columns]
    ...
    if st.sidebar.text_input("Senha Mestra", type="password") == "Workaround&97146605":
        ...
        st.write("---")
        st.write("Lotes:")
        st.dataframe(pd.DataFrame(conectar_google().worksheet("Perfis_Lotes").get_all_records()))
        c1, c2 = st.columns(2)
